...
// s_spi_legs[] table. NOTE: the RX interrupt vectors are EXPLICIT C (_DMA0Interrupt /
// _DMA2Interrupt), each bound to its RX-DMA channel by a compile-time assert -- they are NOT
// token-paste-generated from these macros. So with DSPIC33AK_TDM_DEFINE_DMA_VECTORS=1 (default)
// the RX-DMA channels are effectively FIXED at 0 (SPI1) / 2 (SPI2): changing e.g.
// DSPIC33AK_TDM_SPI1_RX_DMA to 4 fails the build on the assert. To use other RX channels, set
...
// geometry (DSPIC33AK_TDM_SLOTS_PER_FS / _BLOCK_FRAMES). The number of legs is chosen by
// DSPIC33AK_TDM_USE_SPI2 (1 or 2 SPI audio transports). Adding a further physical SPI leg
// is a source edit in the core (enum value + buffer pair + s_spi_legs[] row +
// _DMA<rx>Interrupt vector) plus its channel #defines above -- there is no macro row list.
